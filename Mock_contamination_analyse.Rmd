...
metadata = read.delim("Wetsus_data/KJOH_proj04_Q17535/KJOH_proj04_Q17535_metadata.tsv", header=T, sep = "\t")
colnames(metadata)[1] =  gsub(colnames(metadata)[1], pattern = "X.", "SampleID")
metadata = sample_data(metadata)
sample_names(metadata) = metadata$SampleID

physeq_KJOH_proj04_Q17535 = qza_to_phyloseq(
  features = "Wetsus_data/KJOH_proj04_Q17535/KJOH_16S_515F_926R_08052024_Q17535_full_KJOH_table.qza",
  tree = "Wetsus_data/KJOH_proj04_Q17535/KJOH_16S_515F_926R_08052024_Q17535_full_KJOH_rooted-tree.qza",
  taxonomy = "Wetsus_data/KJOH_proj04_Q17535/KJOH_16S_515F_926R_08052024_Q17535_full_KJOH_silva-138-99-nb-classifier.qza",
  )
...
metadata = read.delim("Wetsus_data/KJOH_proj05_Q17536/KJOH_proj05_Q17536_metadata.tsv", header=T, sep = "\t")
colnames(metadata)[1] =  gsub(colnames(metadata)[1], pattern = "X.", "SampleID")
metadata = sample_data(metadata)
sample_names(metadata) = metadata$SampleID

physeq_KJOH_proj05_Q17536 = qza_to_phyloseq(
  features = "Wetsus_data/KJOH_proj05_Q17536/KJOH_16S_515F_926R_08052024_Q17536_full_KJOH_table.qza",
  tree = "Wetsus_data/KJOH_proj05_Q17536/KJOH_16S_515F_926R_08052024_Q17536_full_KJOH_rooted-tree.qza",
  taxonomy = "Wetsus_data/KJOH_proj05_Q17536/KJOH_16S_515F_926R_08052024_Q17536_full_KJOH_silva-138-99-nb-classifier.qza",
  )
...
metadata = read.delim("Wetsus_data/KJOH_proj06_Q17537/KJOH_proj06_Q17537_metadata.tsv", header=T, sep = "\t")
colnames(metadata)[1] =  gsub(colnames(metadata)[1], pattern = "X.", "SampleID")
metadata = sample_data(metadata)
sample_names(metadata) = metadata$SampleID

physeq_KJOH_proj06_Q17537 = qza_to_phyloseq(
  features = "Wetsus_data/KJOH_proj06_Q17537/KJOH_16S_515F_926R_08052024_Q17537_full_KJOH_table.qza",
  tree = "Wetsus_data/KJOH_proj06_Q17537/KJOH_16S_515F_926R_08052024_Q17537_full_KJOH_rooted-tree.qza",
  taxonomy = "Wetsus_data/KJOH_proj06_Q17537/KJOH_16S_515F_926R_08052024_Q17537_full_KJOH_silva-138-99-nb-classifier.qza",
  )
...
measured_vs_theoretical_inhouse_wetsus_2022 =
  ...
  geom_mark_ellipse(aes(group = Genus, label = Genus), label.fontsize = 9, 
                    expand = unit(3, "mm")) +
...
all_studies_plot_prevalence_median_prevalence =
  ...
  labs(y="Percentage mock OTU in sample")
...
all_studies_plot_previalence =
  ...
  stat_smooth(aes(group = study, color = study), method = "lm", se = T) +
  facet_wrap(~sample_char, nrow = 2, ncol = 4, scales = "free") +
  labs(y="Percentage Sample OTU in mock community")
...
diff_abund_dna_char_plot_prevalence_median =
  ...
  geom_point(aes(color = dna_char, shape = sample_char)) +
  geom_smooth(method="lm", se=T) +
  labs(x = "Difference DNA concentration (mock-sample) (ng/µl)", y = "trend") +
  facet_grid(rows = vars(dna_char), cols = vars(sample_char))
...
# codes die ik niet voor mijn verslag ga gebruiken
...
# inhouse_wetsus_2022
Inhouse_wetsus_2022_studies_samples_dnadiff <-
  Inhouse_wetsus_2022_studies %>% 
  lapply(function(df) {
    mock_dna_concentration_inhouse_wetsus_2022 =
      ...
      summarise(mean_abundance = mean(Abundance), 
            ...
            .groups = "drop") %>% 
      distinct() %>% 
      group_by(study, Sample, Genus) %>%   
      summarise(abund = sum(mean_abundance),
                readcount = mean(mean_read_count),
                dnaconc = mean(mean_dna_conc),
                dnadiff_ms = mock_dna_concentration_inhouse_wetsus_2022 - dnaconc,
                .groups = "drop") %>%  
      arrange(-abund)
    }) %>% 
  bind_rows()
...
# inhouse_wetsus_2023
Inhouse_wetsus_2023_studies_samples_dnadiff <-
  Inhouse_wetsus_2023_studies %>% 
  lapply(function(df) {
    mock_dna_concentration_inhouse_wetsus_2023 =
      df %>% 
      filter(Sample_mock == "mock") %>% 
      filter(!Sample %in% c("MCZ")) %>%  
      ...
      summarise(abund = sum(mean_abundance),
            readcount = sum(mean_read_count),
            dnaconc = mean(mean_dna_conc),
            dnadiff_ms = mock_dna_concentration_inhouse_wetsus_2023 - dnaconc,
            .groups = "drop") %>% 
      arrange(-abund)
    }) %>% 
  bind_rows()
...
# zymo_log10
Zymo_log10_studies_samples_dnadiff <-
  Zymo_log10_studies %>% 
  lapply(function(df) {
    mock_dna_concentration_zymo_log10 =
      ...
      summarise(abund = sum(mean_abundance),
            readcount = sum(mean_read_count),
            dnaconc = mean(mean_dna_conc),
            dnadiff_ms = mock_dna_concentration_zymo_log10 - dnaconc,
            .groups = "drop") %>% 
      arrange(-abund)
    }) %>% 
  bind_rows()
...
# zymo_equil
Zymo_equil_studies_samples_dnadiff <-
  Zymo_equil_studies %>% 
  lapply(function(df) {
    mock_dna_concentration_zymo_equil =
      ...
      summarise(abund = sum(mean_abundance),
            readcount = sum(mean_read_count),
            dnaconc = mean(mean_dna_conc),
            dnadiff_ms = mock_dna_concentration_zymo_equil - dnaconc,
            .groups = "drop") %>% 
      arrange(-abund)
    }) %>% 
  bind_rows()
...
```{r view varation in genus, eval=TRUE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE, cache=TRUE}
# to view variation in genus abundances across samples:
...
# plotting results 
table2_inhouse_wetsus_2022_samples_dnadiff =
  Inhouse_wetsus_2022_studies_samples_dnadiff %>% 
  filter(Genus %in% Inhouse2022_correct$Genus) %>%
  select(study, Genus, abund, dnadiff_ms)

table2_inhouse_wetsus_2023_samples_dnadiff =
Inhouse_wetsus_2023_studies_samples_dnadiff %>%
  filter(Genus %in% Inhouse2023$Genus) %>%
  select(study, Genus, abund, dnadiff_ms)

table2_zymo_log10_samples_dnadiff =
Zymo_log10_studies_samples_dnadiff %>%
  filter(Genus %in% Zymo_log10$Genus) %>%
  select(study, Genus, abund, dnadiff_ms)

table2_zymo_equil_samples_dnadiff =
Zymo_equil_studies_samples_dnadiff %>%
  filter(Genus %in% Zymo_equil$Genus) %>%
  select(study, Genus, abund, dnadiff_ms)
```

```{r test plot, eval=TRUE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE, cache=TRUE}
# Test plot 
# plot inhouse_wetsus_2022
table2_inhouse_wetsus_2022_samples_dnadiff_2 = 
  table2_inhouse_wetsus_2022_samples_dnadiff %>% 
  mutate(log10_abund = log10(abund + 0.0000001),
         sqrt_abund = sqrt(abund + 0.0000001))

# overall linear regression test (Genus random variable)
Statistic_inhouse_wetsus_2022_samples =
  table2_inhouse_wetsus_2022_samples_dnadiff_2 %>% 
  with(lmerTest::lmer(sqrt_abund ~ dnadiff_ms + (1 | Genus)))

summary_model_inhouse_wetsus_2022 <- summary(Statistic_inhouse_wetsus_2022_samples)
coefficients_inhouse_wetsus_2022 <- summary_model_inhouse_wetsus_2022$coefficients

p_value_inhouse_wetsus_2022 <- coefficients_inhouse_wetsus_2022["dnadiff_ms", "Pr(>|t|)"]
correlation_inhouse_wetsus_2022 <- cor(table2_inhouse_wetsus_2022_samples_dnadiff_2$dnadiff_ms,
                                        table2_inhouse_wetsus_2022_samples_dnadiff_2$log10_abund)

# overall linear regression test (Genus-specific)
Statistic_inhouse_wetsus_2022_samples_Genus_int =
  table2_inhouse_wetsus_2022_samples_dnadiff_2 %>% 
  with(lm(sqrt_abund ~ dnadiff_ms*Genus))

hist(resid(Statistic_inhouse_wetsus_2022_samples_Genus_int))

# test significance of interaction
anova(Statistic_inhouse_wetsus_2022_samples_Genus_int)

## interaction not significant: interpret p values from LmerTest
anova(Statistic_inhouse_wetsus_2022_samples)
summary(Statistic_inhouse_wetsus_2022_samples)


inhouse_wetsus_2022_samples_dnadiff =
  table2_inhouse_wetsus_2022_samples_dnadiff %>% 
  ggplot(aes(x = dnadiff_ms, y = abund)) + 
  ...
  labs(x = 'DNA Concentration difference (Mock - Sample)', y = 'Percentage Mock OTU in sample (sqrt transformed) ') +
  annotate("text", x = -Inf, y = Inf, label = paste0("P-value: ", format.pval(p_value_inhouse_wetsus_2022),
                                                     "\nCorrelation: ", round(correlation_inhouse_wetsus_2022, 3)), 
           hjust = -0.1, vjust = 1.1, size = 3.5, color = "black")


ggsave("figures/inhouse_wetsus_2022_samples_dnadiff.pdf", plot = inhouse_wetsus_2022_samples_dnadiff, width = 6, height = 4)
ggsave("figures/inhouse_wetsus_2022_sampels_dnadiff.jpg", plot = inhouse_wetsus_2022_samples_dnadiff, width = 6, height = 4)


```

```{r Calculate statistics and plot abundance against mock-sample DNA concentration (sampels), eval=TRUE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE, cache=TRUE}
# statastic inhouse_wetsus_2022 

table3_inhouse_wetsus_2022_samples_dnadiff = 
  table2_inhouse_wetsus_2022_samples_dnadiff %>% 
  mutate(log10_abund = log10(abund + 0.0000001),
         sqrt_abund = sqrt(abund + 0.0000001))
...
p_value_inhouse_wetsus_2022 <- coefficients_inhouse_wetsus_2022["dnadiff_ms", "Pr(>|t|)"]
correlation_inhouse_wetsus_2022 <- cor(table2_inhouse_wetsus_2022_samples_dnadiff_2$dnadiff_ms,
                                        table2_inhouse_wetsus_2022_samples_dnadiff_2$sqrt_abund)
...
# statastic inhouse_wetsus_2023
table3_inhouse_wetsus_2023_samples_dnadiff = 
  table2_inhouse_wetsus_2023_samples_dnadiff %>% 
  mutate(log10_abund = log10(abund + 0.0000001),
         sqrt_abund = sqrt(abund + 0.0000001))

# overall linear regression test (Genus random variable)
Statistic_inhouse_wetsus_2023_samples =
  table3_inhouse_wetsus_2023_samples_dnadiff %>% 
  with(lmerTest::lmer(log10_abund ~ dnadiff_ms + (1 | Genus)))

summary_model_inhouse_wetsus_2023 <- summary(Statistic_inhouse_wetsus_2023_samples)
coefficients_inhouse_wetsus_2023 <- summary_model_inhouse_wetsus_2023$coefficients

p_value_inhouse_wetsus_2023 <- coefficients_inhouse_wetsus_2023["dnadiff_ms", "Pr(>|t|)"]
correlation_inhouse_wetsus_2023 <- cor(table3_inhouse_wetsus_2023_samples_dnadiff$dnadiff_ms,
                                        table3_inhouse_wetsus_2023_samples_dnadiff$log10_abund)

# overall linear regression test (Genus-specific)
Statistic_inhouse_wetsus_2023_samples_Genus_int =
  table3_inhouse_wetsus_2023_samples_dnadiff %>% 
  with(lm(log10_abund ~ dnadiff_ms*Genus))

hist(resid(Statistic_inhouse_wetsus_2023_samples_Genus_int))

# test significance of interaction
anova(Statistic_inhouse_wetsus_2023_samples_Genus_int)

## interaction not significant: interpret p values from LmerTest
anova(Statistic_inhouse_wetsus_2023_samples)
summary(Statistic_inhouse_wetsus_2023_samples)

# ##
# genus_models = 
#   table3_inhouse_wetsus_2023_samples_dnadiff %>%
#   group_by(Genus) %>%
#   do(model = lm(log10_abund ~ dnadiff_ms, data = .))
# 
# genus_p_values =
#   genus_models %>%
#   summarise(Genus = Genus, p_value = summary(model)$coefficients["dnadiff_ms", "Pr(>|t|)"])


# Plot inhouse_wetsus_2023
inhouse_wetsus_2023_samples_dnadiff =
  table2_inhouse_wetsus_2023_samples_dnadiff %>% 
  ggplot(aes(x = dnadiff_ms, y = abund)) + 
  geom_point(aes(color = Genus), position = position_dodge(width = 0.5)) + 
  #facet_wrap(~Genus) +
  ...
  geom_smooth(method = "lm", se = F, color = "grey50") +
  theme(axis.text.x = element_text(size = 8)) +
  labs(x = 'DNA Concentration difference (Mock - Sample)', y = 'Percentage Mock OTU in sample (log10 transformed) ') +
  # geom_text(data = genus_p_values, aes(x = -Inf, y = Inf, 
  #                                     label = paste0("p_value: ", format.pval(p_value, 
  #                                                                             scientific = F,
  #                                                                             digits = 3))),
  #          hjust = -0.1, vjust = 1.1, size = 3.5, color = "black") 
  annotate("text", x = -Inf, y = Inf, label = paste0("P-value: ", format.pval(p_value_inhouse_wetsus_2023, 
                                                                              scientific = T, 
                                                                              digits = 3),
                                                     "\nCorrelation: ", round(correlation_inhouse_wetsus_2023, 3)), 
           hjust = -0.1, vjust = 1.1, size = 3.5, color = "black")


ggsave("figures/inhouse_wetsus_2023_samples_dnadiff.pdf", plot = inhouse_wetsus_2023_samples_dnadiff, width = 6, height = 4)
ggsave("figures/inhouse_wetsus_2023_samples_dnadiff.jpg", plot = inhouse_wetsus_2023_samples_dnadiff, width = 6, height = 4)

inhouse_wetsus_2023_samples_dnadiff

# statastic zymo_log10
table3_zymo_log10_samples_dnadiff = 
  table2_zymo_log10_samples_dnadiff %>% 
  mutate(log10_abund = log10(abund + 0.0000001),
         sqrt_abund = sqrt(abund + 0.0000001))

# overall linear regression test (Genus random variable)
Statistic_zymo_log10_samples =
  table3_zymo_log10_samples_dnadiff %>% 
  with(lmerTest::lmer(sqrt_abund ~ dnadiff_ms + (1 | Genus)))

summary_model_zymo_log10 <- summary(Statistic_zymo_log10_samples)
coefficients_zymo_log10 <- summary_model_zymo_log10$coefficients

p_value_zymo_log10 <- coefficients_zymo_log10["dnadiff_ms", "Pr(>|t|)"]
correlation_zymo_log10 <- cor(table3_zymo_log10_samples_dnadiff$dnadiff_ms,
                              table3_zymo_log10_samples_dnadiff$sqrt_abund)

# overall linear regression test (Genus-specific)
Statistic_zymo_log10_samples_Genus_int =
  table3_zymo_log10_samples_dnadiff %>% 
  with(lm(sqrt_abund ~ dnadiff_ms*Genus))

hist(resid(Statistic_zymo_log10_samples_Genus_int))

# test significance of interaction
anova(Statistic_zymo_log10_samples_Genus_int)

## interaction not significant: interpret p values from LmerTest
anova(Statistic_zymo_log10_samples)
summary(Statistic_zymo_log10_samples)


# plot zymo log10
zymo_log10_samples_dnadiff =
  table2_zymo_log10_samples_dnadiff %>% 
ggplot(aes(x = dnadiff_ms, y = abund)) + 
  geom_point(aes(color = Genus), position = position_dodge(width = 0.5)) + 
  #facet_wrap(~Genus) +
  ...
  theme(axis.text.x = element_text(size = 8)) +
  labs(x = 'DNA Concentration difference (Mock - Sample)', y = 'Percentage Mock OTU in sample (sqrt transformed) ') +
  annotate("text", x = -Inf, y = Inf, label = paste0("P-value: ", format.pval(p_value_zymo_log10, 
                                                                              scientific = T,
                                                                              digits = 3),
                                                     "\nCorrelation: ", round(correlation_zymo_log10, 3)), 
           hjust = -0.1, vjust = 1.1, size = 3.5, color = "black")

ggsave("figures/zymo_log10_samples_dnadiff.pdf", plot = zymo_log10_samples_dnadiff, width = 6, height = 4)
ggsave("figures/zymo_log10_samples_dnadiff.jpg", plot = zymo_log10_samples_dnadiff, width = 6, height = 4)

zymo_log10_samples_dnadiff

# statastic zymo_equil
table3_zymo_equil_samples_dnadiff = 
  table2_zymo_equil_samples_dnadiff %>% 
  mutate(log10_abund = log10(abund + 0.0000001),
         sqrt_abund = sqrt(abund + 0.0000001))

# overall linear regression test (Genus random variable)
Statistic_zymo_equil_samples =
  table3_zymo_equil_samples_dnadiff %>% 
  with(lmerTest::lmer(log10_abund ~ dnadiff_ms + (1 | Genus)))

summary_model_zymo_equil <- summary(Statistic_zymo_equil_samples)
coefficients_zymo_equil <- summary_model_zymo_equil$coefficients

p_value_zymo_equil <- coefficients_zymo_equil["dnadiff_ms", "Pr(>|t|)"]
correlation_zymo_equil <- cor(table3_zymo_equil_samples_dnadiff$dnadiff_ms,
                              table3_zymo_equil_samples_dnadiff$log10_abund)

# overall linear regression test (Genus-specific)
Statistic_zymo_equil_samples_Genus_int =
  table3_zymo_equil_samples_dnadiff %>% 
  with(lm(log10_abund ~ dnadiff_ms*Genus))

hist(resid(Statistic_zymo_equil_samples_Genus_int))

# test significance of interaction
anova(Statistic_zymo_equil_samples_Genus_int)

## interaction not significant: interpret p values from LmerTest
anova(Statistic_zymo_equil_samples)
summary(Statistic_zymo_equil_samples)


# plot zymo equil
plot_zymo_equil_samples_dnadiff =
  table2_zymo_equil_samples_dnadiff %>% 
  ggplot(aes(x = dnadiff_ms, y = abund)) + 
  geom_point(aes(color = Genus), position = position_dodge(width = 0.5)) + 
  #facet_wrap(~Genus) +
  ...
  theme(axis.text.x = element_text(size = 8)) +
  labs(x = 'DNA Concentration difference (Mock - Sample)', y = 'Percentage Mock OTU in sample (log10 transformed) ') +
  annotate("text", x = -Inf, y = Inf, label = paste0("P-value: ", format.pval(p_value_zymo_equil, 
                                                                              scientific = T,
                                                                              digits = 3),
                                                     "\nCorrelation: ", round(correlation_zymo_equil, 3)), 
           hjust = -0.1, vjust = 1.1, size = 3.5, color = "black")
...
```{r statistiek (sampels), eval=TRUE, echo=TRUE, include=TRUE, message=FALSE, warning=FALSE, cache=TRUE}
# statistic 
Statistic_inhouse_wetsus_2022_samples =
  table2_inhouse_wetsus_2022_samples_ratio %>% 
  mutate(log2_abund = log2(abund+0.0000001)) %>% 
  with(lmerTest::lmer(log2_abund ~ dnaratio_ms + (1|Genus))) %>%
  anova

Statistic_inhouse_wetsus_2022_samples

#lm(abund ~ dnaconc, data = table2_inhouse_wetsus_2022_samples_ratio)
